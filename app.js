...
// ============================================================
// 🔐 אבטחה — שנה את הסיסמה כאן
// ============================================================
var ADMIN_PASS = 'cashphone2026!';
var MAX_LOGIN_ATTEMPTS = 5;
var LOCKOUT_MINUTES = 15;

var _loginAttempts = {};
function _checkRateLimit(username){
  var key='rl_'+username;
  var now=Date.now();
  if(!_loginAttempts[key])_loginAttempts[key]={count:0,firstAttempt:now,locked:false};
  var r=_loginAttempts[key];
  if(now-r.firstAttempt>LOCKOUT_MINUTES*60*1000){_loginAttempts[key]={count:0,firstAttempt:now,locked:false};return{allowed:true};}
  if(r.locked){var remaining=Math.ceil((LOCKOUT_MINUTES*60*1000-(now-r.firstAttempt))/60000);return{allowed:false,remaining:remaining};}
  return{allowed:true};
}
function _recordFailedAttempt(username){
  ...
  if(_loginAttempts[key].count>=MAX_LOGIN_ATTEMPTS)_loginAttempts[key].locked=true;
}
function _resetAttempts(username){ delete _loginAttempts['rl_'+username]; }
// ============================================================
...
function doLogin(autoLoginUser){
  ...
  if(autoLoginUser){
    ...
    // Rate limiting — רק על משתמשים רגילים, לא על אדמין
    if(u!=='admin'){
      var rl=_checkRateLimit(u);
      if(!rl.allowed){
        var errEl=document.getElementById('login-err');
        errEl.classList.add('on');
        errEl.textContent='🔒 חשבון נעול ל-'+rl.remaining+' דקות';
        document.getElementById('li-pass').value='';
        return;
      }
    }
